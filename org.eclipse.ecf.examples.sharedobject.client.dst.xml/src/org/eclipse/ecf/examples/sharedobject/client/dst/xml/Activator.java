package org.eclipse.ecf.examples.sharedobject.client.dst.xml;
...
public class Activator implements BundleActivator
{
	...
	private IContainer[] createClients() throws Exception
	{
		...
		for (int i = 0; i < _containers.length; i++)
			getClientSOContainer(i).setSharedObjectMessageSerializer(new XStreamSOMessageSerializer());
	...
	{
		for (int i = 0; i < _containers.length; i++)
			connectClient(_containers[i], createServerID(), null);
	}
	...
	{
		return ContainerFactory.getDefault().createContainer(GENERIC_CLIENT_CONTAINER);
	}
